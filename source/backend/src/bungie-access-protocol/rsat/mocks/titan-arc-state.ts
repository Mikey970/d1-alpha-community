...
import native from "./titan-arc-native.json";
...
export const TALENT_ITEM_SOID = 0x0000000300000203n;
export const TALENT_ITEM_DEFINITION = 434;
export const TALENT_GRID_INDEX = 243;
...
// Exact grid243 first-acquisition rules are checked during native extraction.
...
function statePath() {
  ...
  return novaPath ? `${novaPath}.titan-arc.json` : undefined;
}
...
function loadPersistedState(): void {
  ...
  if (existsSync(path)) {
    ...
    if (saved.schema !== 2 || saved.itemSoid !== TALENT_ITEM_SOID.toString(16) ||
        ...
        !["client-801-with-explicit-test-progression", "user-requested-titan-arc-test"].includes(String(saved.provenance)) ||
        !Number.isSafeInteger(saved.version) || Number(saved.version) < 1) {
      throw new Error("Invalid persisted Titan talent test state");
    }
...
export function talentValue(): rsat.Infer<typeof Unknown808019AE> {
  ...
  // Native836BDA50 grants these three nodes for grid243.
...
export function activateTalentStep(itemSoid: bigint, nodeIndex: number, swap=false): TalentMutationResult {
  ...
  if (itemSoid !== TALENT_ITEM_SOID) return reject("not-owned-titan-test-item");
  ...
  return { accepted: true, changed: true, reason: `titan-grid243-node${nodeIndex}-step0-slot${node.slot}` };
}
export function swapTalentNode(itemSoid: bigint, nodeIndex: number): TalentMutationResult {
  // Explicit upgraded local profile: the existing 802 request identifies the
  // destination node. Replace only its extracted exclusive group selection.
