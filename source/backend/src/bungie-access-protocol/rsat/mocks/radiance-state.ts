...
import native from "./radiance-native.json";
import type { rsat } from "@blamnetwork/rsat";
import type { Unknown808019AE } from "../../queuez/families/self";
import { existsSync, readFileSync, writeFileSync, renameSync } from "node:fs";

export const TALENT_ITEM_SOID = 0x0000000300000201n;
export const TALENT_ITEM_DEFINITION = 441;
export const TALENT_GRID_INDEX = 250;
...
export const TALENT_RADIANCE_NODE = 25;
...
// Exact grid250 first-acquisition rules are checked during native extraction.
...
function statePath() {
  ...
  return novaPath ? `${novaPath}.radiance.json` : undefined;
}
...
function loadPersistedState(): void {
  ...
  if (existsSync(path)) {
    ...
    if (saved.schema !== 2 || saved.itemSoid !== TALENT_ITEM_SOID.toString(16) ||
        ...
        saved.testXp !== TALENT_TEST_XP || !validSelection(saved.purchased) ||
...
export function talentValue(): rsat.Infer<typeof Unknown808019AE> {
  ...
  // Native836BDA50 grants these three nodes for grid250.
...
export function activateTalentStep(itemSoid: bigint, nodeIndex: number, swap = false): TalentMutationResult {
  ...
  return { accepted: true, changed: true, reason: `warlock-grid250-node${nodeIndex}-step0-slot${node.slot}` };
}
