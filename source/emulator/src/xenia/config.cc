...
void SaveConfig() {
  ...
  for (auto config_var : vars) {
    ...
    if (last_category != config_var->category()) {
      ...
      // Quote each path component, preserving nested categories such as
      // HID.WinKey. Bare headers like [D1 Alpha] are invalid TOML.
      sb.Append('[');
      bool first_component = true;
      for (const auto& component : xe::utf8::split(last_category, ".")) {
        if (!first_component) {
          sb.Append('.');
        }
        sb.Append(cvar::toml_internal::EscapeString(component));
