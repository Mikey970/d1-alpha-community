...
export const ROSTER_LENGTHS: Readonly<Record<number, number>> = Object.freeze({2:2,3:2,4:3,5:4,6:4,7:4,8:3,9:2,10:3,11:2,12:2,13:1,14:1,15:1,16:1,17:1,18:1,19:1});
export type StrikeSenseRow = {
  ...
  generation?:number; terminal?:boolean; bound?:boolean;
};
  ...
  try {
    ...
    while (read(1)) {
      ...
      if (type===1) {
        ...
        if (read(1)) for (let i = 0; i < 24; i++) opt(7); // Native 823B0644..654: 24 optional compressed scalars.
      } else if (type===3) {
        ...
        if (read(1)) return undefined; // Actor compressed scalar unknown.
        opt(31);
        if (read(1)) {opt(6);opt(31);opt(31);read(1);}
        opt(31); if (read(2)-1!==0) return undefined; opt(31);
        row.terminal=read(1)===1; row.bound=read(1)===1;
      } else if (type===4) {
        // Global exit0 and exact packaged owner5 Objects13..39 share808005F0
        // (client2306..2332 identity+71C, Sense schema+730). Only this exact
        // Object header and known single device80802D89 are consumed; other
        // component tags/counts still reject the complete packet.
        if (!((bundle===ENGINEERING && index===0) ||
              (bundle===0x5de7d8e2 && index>=13 && index<=39) ||
              // Private Engineering client2345..2376: identical808005F0 at+730.
              (bundle===0x97438d09 && index>=5 && index<=36))) return undefined;
