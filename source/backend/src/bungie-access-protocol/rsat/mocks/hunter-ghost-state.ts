...
import native from "./hunter-ghost-native.json";
import type { rsat } from "@blamnetwork/rsat";
import type { Unknown808019AE } from "../../queuez/families/self";
import { existsSync, readFileSync, writeFileSync, renameSync } from "node:fs";

export const TALENT_ITEM_SOID = 0x0000000300000204n;
export const TALENT_ITEM_DEFINITION = 438;
export const TALENT_GRID_INDEX = 247;
...
export const TALENT_GHOST_NODE = 25;
...
// Exact grid247 first-acquisition rules are checked during native extraction.
...
function statePath() {
  ...
  return novaPath ? `${novaPath}.hunter-ghost.json` : undefined;
}
...
function loadPersistedState(): void {
  ...
  if (existsSync(path)) {
    ...
    if (saved.schema !== 2 || saved.itemSoid !== TALENT_ITEM_SOID.toString(16) ||
        ...
        !["client-801-with-explicit-test-progression", "user-requested-hunter-ghost-test"].includes(String(saved.provenance)) ||
...
export function talentValue(): rsat.Infer<typeof Unknown808019AE> {
  ...
  // Native836BDA50 grants these three nodes for grid247.
...
export function activateTalentStep(itemSoid: bigint, nodeIndex: number, swap=false): TalentMutationResult {
  ...
  return { accepted: true, changed: true, reason: `hunter-grid247-node${nodeIndex}-step0-slot${node.slot}` };
}
