...
DEFINE_int32(key_press_pulse_polls, 4,
             "Number of controller polls a WinKey key-down pulse remains active "
             "(1-255).",
             "HID.WinKey");

DEFINE_bool(trace_key_pulses, false,
            "Log at most 128 keyboard events and 64 trigger polls for input "
            "diagnosis. Does not change controller state.",
            "HID.WinKey");
...
void WinKeyInputDriver::ParseKeyBinding(ui::VirtualKey output_key,
  ...
  for (const std::string_view source_token :
    ...
    if (utf8::starts_with(token, "0x")) {
      ...
      uint16_t parsed_key = 0;
      const auto [end, error] = std::from_chars(
          token.data(), token.data() + token.size(), parsed_key, 16);
      if (error == std::errc() && end == token.data() + token.size()) {
        key_binding.input_key = static_cast<ui::VirtualKey>(parsed_key);
      }
    ...
    if (key_binding.input_key == ui::VirtualKey::kNone ||
        static_cast<uint16_t>(key_binding.input_key) > 0xFF) {
...
X_RESULT WinKeyInputDriver::GetState(uint32_t user_index,
...

  {
    ...
    const bool has_focus = window()->HasFocus();
    const bool current_capital = has_focus &&
                                 (IsKeyToggled(VK_CAPITAL) || IsKeyDown(VK_SHIFT));
    for (const KeyBinding& b : key_bindings_) {
      const auto key_index = static_cast<uint8_t>(b.input_key);
      const bool held = has_focus && IsKeyDown(b.input_key);
      // A buffered tap must retain its binding after the modifier is released.
      // Otherwise Shift+W (D-pad up) becomes W (move forward) on later polls.
      const bool capital = held ? current_capital : key_press_capital_[key_index];
      if (((b.lowercase == b.uppercase) || (b.lowercase && !capital) ||
           (b.uppercase && capital)) &&
          (held || key_press_pulses_[key_index] != 0)) {
        switch (b.output_key) {
    ...
    if (cvars::trace_key_pulses && trace_trigger_polls_ < 64 &&
        (left_trigger || right_trigger || trace_previous_trigger_)) {
      ++trace_trigger_polls_;
      XELOGI("winkey trigger poll: ms={} packet={} focus={} lt={} rt={} "
             "e_pulse={} o_pulse={}",
             Clock::QueryHostUptimeMillis(), packet_number_, has_focus,
             left_trigger, right_trigger, key_press_pulses_['E'],
             key_press_pulses_['O']);
    }
    trace_previous_trigger_ = left_trigger || right_trigger;
    for (uint8_t& pulse : key_press_pulses_) {
      if (pulse != 0) {
        --pulse;
      }
    }
...
void WinKeyInputDriver::OnKey(ui::KeyEvent& e, bool is_down) {
  ...
  if (cvars::trace_key_pulses && trace_key_events_ < 128) {
    ++trace_key_events_;
    XELOGI("winkey key event: ms={} vk={:X} down={} shift={} ctrl={} alt={}",
           Clock::QueryHostUptimeMillis(), static_cast<uint16_t>(e.virtual_key()),
           is_down, e.is_shift_pressed(), e.is_ctrl_pressed(), e.is_alt_pressed());
  }
  if (is_down) {
    const int32_t requested_polls = cvars::key_press_pulse_polls;
    const uint8_t pulse_polls = static_cast<uint8_t>(
        requested_polls < 1 ? 1 : (requested_polls > 255 ? 255 : requested_polls));
    key_press_pulses_[static_cast<uint8_t>(e.virtual_key())] = pulse_polls;
    key_press_capital_[static_cast<uint8_t>(e.virtual_key())] =
        IsKeyToggled(VK_CAPITAL) || e.is_shift_pressed();
  }
}
